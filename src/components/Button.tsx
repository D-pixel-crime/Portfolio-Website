import { KeyTextField, LinkField } from "@prismicio/client";
import { PrismicNextLink } from "@prismicio/next";
import clsx from "clsx";
import { ExternalLinkIcon } from "lucide-react";

interface ButtonProps {
  link: LinkField;
  label: KeyTextField;
  className?: string;
  showIcon?: boolean;
  buttonColor: string;
}

const Button = ({
  link,
  label,
  showIcon,
  className,
  buttonColor,
}: ButtonProps) => {
  return (
    <PrismicNextLink
      field={link}
      className={clsx(
        "flex-center group relative w-fit overflow-hidden rounded-md border-2 border-slate-900 bg-slate-50 px-4 py-2 font-bold transition-transform ease-out hover:scale-105 no-underline",
        className
      )}
    >
      <span
        className={clsx(
          "absolute inset-0 z-0 h-full translate-x-[-90%] translate-y-[80%] transition-transform duration-300 ease-in-out group-hover:translate-x-0 group-hover:translate-y-0",
          buttonColor
        )}
      ></span>

      <span className="relative text-slate-900 flex-center gap-2">
        {label} {showIcon && <ExternalLinkIcon className="inline-block" />}
      </span>
    </PrismicNextLink>
  );
};

export default Button;
